...
def experiment(
    ...
    seed = variant['seed']
    exp_prefix = f'{group_name}/{opt}-{model_type}-{seed}-{timestr}'
    ...
    setup_logger(variant=variant, log_dir=os.path.join(variant['save_path'], exp_prefix))
    writer = SummaryWriter(log_dir=os.path.join(variant['save_path'], exp_prefix))
    ...
    # load dataset
    dataset_path = f'data/d4rl/{env_name}-{dataset}-v2.pkl'
    ...
    for iter in range(variant['epochs']):
        ...
        if ret > best_ret:
            ...
            save_checkpoint(state, os.path.join(variant['save_path'], exp_prefix, 'best_model.pth'))
            best_ret = ret

    logger.log(f'The final best return mean is {best_ret}, corresponding normalize score is {env.get_normalized_score(best_ret)}')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--save_path', type=str, default='./runs/DT/')
    ...
    experiment('Covergence', variant=vars(args))
