...
class Trainer:
    ...
    def merge_model(self, num_steps, iter_num, step, logger):
        ...
        ada_optimizer = torch.optim.AdamW(ada_model.collect_trainable_params(), lr=self.variant['learning_rate'])

        ada_model.train()
        for merge_epoch in range(self.variant['merge_epoch']):
            for i in trange(num_steps, desc=f"Epoch {iter_num}, Step {step}, Merge epoch {merge_epoch}", leave=False):
                train_loss = self.train_step(ada_model, ada_optimizer)
                train_losses.append(train_loss)
            eval_logits.append(ada_model.mask_logit.detach().cpu().numpy()) 
        ...
        logger.log(f'epoch {iter_num}, Step {step}')
